...
-- Canonical source SHA-256: 6cda75d1aec8e12326662fe2c2db017c3be7d012ce961d9d1bef0c6d1748c044
DO $$
...
RETURNS UUID AS $$
    SELECT shop_id
    FROM staff_users
    WHERE id = ps01_request_user_id() AND is_active = TRUE;
...
RETURNS BOOLEAN AS $$
    SELECT EXISTS (
        ...
        WHERE id = ps01_request_user_id() AND is_active = TRUE AND role = 'owner'
    );
$$ LANGUAGE sql STABLE SECURITY DEFINER SET search_path = ps01, extensions, pg_temp;
...
RETURNS BOOLEAN AS $$
    SELECT EXISTS (
        SELECT 1 FROM staff_users
        WHERE id = ps01_request_user_id() AND is_active = TRUE AND role IN ('owner', 'manager')
    );
$$ LANGUAGE sql STABLE SECURITY DEFINER SET search_path = ps01, extensions, pg_temp;
...
DECLARE
    ...
    v_context JSONB;
BEGIN
    v_caller_id := ps01_request_user_id();
...
-- Managed Storage bucket mutation emitted separately for platform-admin apply.
...
BEGIN
    v_shop_id := current_staff_shop_id();
    v_actor_id := ps01_request_user_id();
...
BEGIN
    ...
    v_shop_id := current_staff_shop_id();
    v_user_id := ps01_request_user_id();
...
BEGIN
    ...
    SET status = 'confirmed',
        ...
        actioned_by = ps01_request_user_id(),
        actioned_at = now()
    WHERE id = p_request_id AND shop_id = v_shop_id;
...
BEGIN
  ...
  END IF;
  SELECT * INTO v_staff FROM staff_users WHERE id=ps01_request_user_id() AND shop_id=v_shop_id AND is_active=TRUE;
...
-- BEGIN LEGACY SOURCE: 20260908103100_booking_v2_rate_plans.sql
...
GRANT SELECT ON TABLE room_rate_plans TO authenticated, ps01_runtime;
...
BEGIN
    ...
    END CASE;
END;
$$ LANGUAGE plpgsql IMMUTABLE SET search_path = ps01, extensions, pg_temp;
...
GRANT EXECUTE ON FUNCTION create_room_rate_plan(UUID, VARCHAR, INT, NUMERIC) TO authenticated, ps01_runtime;
...
GRANT EXECUTE ON FUNCTION update_room_rate_plan(UUID, VARCHAR, INT, NUMERIC, BOOLEAN) TO authenticated, ps01_runtime;
...
GRANT EXECUTE ON FUNCTION quote_booking_v2(UUID, UUID, UUID, UUID[], TIMESTAMPTZ) TO authenticated, ps01_runtime;
...
GRANT EXECUTE ON FUNCTION create_booking_v2(UUID, UUID, UUID, UUID[], TIMESTAMPTZ, TEXT) TO authenticated, ps01_runtime;
...
GRANT EXECUTE ON FUNCTION update_booking_v2_schedule(UUID, UUID, UUID, TIMESTAMPTZ, TEXT) TO authenticated, ps01_runtime;
...
BEGIN
    ...
    SET status = 'confirmed', confirmed_booking_id = v_booking_id,
        actioned_by = ps01_request_user_id(), actioned_at = now()
    WHERE id = p_request_id AND shop_id = v_shop_id;
...
GRANT EXECUTE ON FUNCTION set_room_maintenance_v2(UUID, TIMESTAMPTZ, TIMESTAMPTZ) TO authenticated, ps01_runtime;
...
REVOKE ALL ON FUNCTION create_booking(UUID, UUID, DATE, DATE, NUMERIC, TEXT)
    FROM PUBLIC, anon, authenticated, ps01_runtime;
REVOKE ALL ON FUNCTION submit_booking_request_internal(VARCHAR, UUID, UUID, UUID[], DATE, DATE, TEXT)
    FROM PUBLIC, anon, authenticated, ps01_runtime;
...
-- END LEGACY SOURCE: 20260908103100_booking_v2_rate_plans.sql
...
VALUES ('6cda75d1aec8e12326662fe2c2db017c3be7d012ce961d9d1bef0c6d1748c044', 14)
